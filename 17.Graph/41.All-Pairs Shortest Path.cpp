 ...
 *
 * Key Observations:
 * - Computes shortest paths from every node to every other node (all-pairs).
 * - Handles negative edge weights correctly.
 * - Detects negative cycles if any dist[i][i] < 0.
 * - Uses dynamic programming by iteratively considering each vertex as an intermediate node.
 *
 * Intuition:
